...
describe('pool math', () => {
  it('should compute collateral balance from blockchain data', () => {
    ...
    // reserve: YFI
    const scaledLTokenBalance = '161316503206059870';
    ...
    const expectedLTokenBalance = '161594727054623229';
    const underlyingBalance = getLinearBalance({
      balance: scaledLTokenBalance,
      index: liquidityIndex,
      rate: currentLiquidityRate,
      lastUpdateTimestamp,
      currentTimestamp,
    }).toString();
    expect(underlyingBalance).toEqual(expectedLTokenBalance);
  });
